...
## 16. WebView2 在 ShowDialog 模态窗口中的初始化

**现象：** SSO 登录弹窗 `ShowDialog()` 后显示空白窗口，WebView2 不加载页面。

**根因：** 在 `Load` 事件中用 `async void` 初始化 WebView2（`EnsureCoreWebView2Async`），然后 `ShowDialog()` 启动模态消息循环。`ShowDialog` 的模态消息循环与 `WindowsFormsSynchronizationContext` 的续体调度不兼容——async continuation 无法被正确 dispatch，WebView2 初始化永远完不成。

**修复：** 在调用 `ShowDialog()` 之前完成 WebView2 初始化：

```csharp
// ❌ 错误：Load 事件中 async void + ShowDialog 消息循环冲突
public SsoLoginPopup() {
    Load += async (s, e) => {
        await webView.EnsureCoreWebView2Async(environment); // 永远不会完成
        webView.CoreWebView2.Navigate(ssoUrl);
    };
}
dialog.ShowDialog(); // 模态消息循环阻止 async 续体

// ✅ 正确：ShowDialog 之前完成初始化
var popup = new SsoLoginPopup(ssoUrl, sharedCookies, cookieStore);
await popup.InitializeAsync(); // 在 ShowDialog 之前 await 完成
popup.ShowDialog();
```

**教训：** `ShowDialog()` 的模态消息循环会干扰 async/await 续体调度。涉及 WebView2 初始化的 Form，必须在 `ShowDialog()` 之前用 `await` 完成初始化，不要依赖 `Load` 事件中的 `async void`。

## 17. Uri.Host 不含端口号——SSO Cookie 捕获的 Origin 比较陷阱

**现象：** SSO 服务在 `localhost:3100`，业务 API 在 `localhost:3200`。SSO 登录后 302 重定向到业务 API，但 `SsoLoginPopup` 不触发 cookie 捕获。

**根因：** 用 `Uri.Host` 比较 origin，`localhost:3100` 和 `localhost:3200` 的 Host 都是 `localhost`，比较结果相等，不会触发 cookie 捕获逻辑。

**修复：** 改用 `Uri.Authority`（包含 host:port）：

```csharp
// ❌ 错误：Host 不含端口，localhost:3100 == localhost:3200
var ssoDomain = new Uri(ssoUrl).Host;
var currentHost = new Uri(webView.CoreWebView2.Source).Host;

// ✅ 正确：Authority 含端口，localhost:3100 != localhost:3200
var ssoAuthority = new Uri(ssoUrl).Authority;
var currentAuthority = new Uri(webView.CoreWebView2.Source).Authority;
```

**教训：** 当 SSO 和业务 API 在同一主机不同端口时（本地开发常见），origin 比较必须用 `Uri.Authority`（`host:port`），不能用 `Uri.Host`。生产环境如果 SSO 和 API 域名完全不同（如 `sso.company.com` vs `api.company.com`），两者都行，但 `Authority` 是更通用的正确选择。
