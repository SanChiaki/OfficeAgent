...
namespace OfficeAgent.Infrastructure.Http
{
    public sealed class AiColumnMappingClient : IAiColumnMappingClient
    {
        ...
        {
            return MapAsync(request, CancellationToken.None).GetAwaiter().GetResult();
        }

        public async Task<AiColumnMappingResponse> MapAsync(AiColumnMappingRequest request)
        {
            return await MapAsync(request, CancellationToken.None).ConfigureAwait(false);
        }

        public async Task<AiColumnMappingResponse> MapAsync(AiColumnMappingRequest request, CancellationToken cancellationToken)
        {
            if (request == null)
            {
            ...
            var mappingJson = await TrySendStreamingRequestAsync(
                endpoint,
                settings.ApiKey,
                settings.Model,
                request,
        ...
        private async Task<string> TrySendStreamingRequestAsync(
            Uri endpoint,
            string apiKey,
            string model,
            ...
            CancellationToken cancellationToken)
        {
            using (var httpRequest = CreateRequest(endpoint, apiKey, BuildPayload(model, request, stream: true, apiFormat), apiFormat))
            {
                using (var response = await httpClient.SendAsync(httpRequest, HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false))
                {
                    ...
                    return await ReadStreamingMappingTextAsync(response, apiFormat, cancellationToken).ConfigureAwait(false);
                }
            }
        }
        ...
        private static async Task<string> ReadStreamingMappingTextAsync(
