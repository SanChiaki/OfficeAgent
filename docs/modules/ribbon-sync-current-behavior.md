...
所有 Ribbon 按钮都使用 Office 内置 `imageMso` 图标，并按按钮语义选择图标。`初始化当前表` / `Initialize sheet`、`AI映射列` / `AI map columns`、`配置` / `Setting` 组内的三个命令按钮以及账号组内的 `登录` / `Login`、`登出` / `Logout` 使用 Office 常规小按钮布局；数据同步、帮助等其余命令按钮使用 Office 大按钮布局，图标显示在上方、文字显示在下方。项目下拉框仍然只显示当前项目或状态文本。
