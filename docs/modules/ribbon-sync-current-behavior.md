...
插件启动后会在后台以非阻塞方式检查新版本；Debug 和 Release 使用同一套更新检查逻辑，不再按构建配置区分是否请求更新源。检查源是内部配置的独立 URL，不复用业务后端，也不显示在任务窗格设置页。发现比当前安装版本更高的新版本且用户未忽略该版本时，`关于` / `About` 图标会显示红点。点击 `关于` 会展示当前版本、最新版本、更新摘要和下载入口，并可选择 `忽略此版本` / `Ignore this version`；忽略后只隐藏红点，`关于` 弹框仍保留该版本的更新信息和下载入口，方便用户之后手动更新。`关于` 弹框复用下载 / 上传确认弹框的宿主对话框壳，保持一致的字体、图标、边距和按钮样式。没有新版本时显示 `当前已是最新版本。` / `You are on the latest version.`。如果没有配置更新 manifest URL，则不会发起更新请求。更新检查失败只写本地日志，不弹窗，也不影响 Ribbon、任务窗格、登录、同步或模板操作。
...
账号组按钮状态以服务端会话状态为准，不再根据本地 Cookie 数量判断。插件会在 Ribbon 绑定、登录后、项目列表加载成功以及业务接口返回 `401/403` 时同步账号状态：服务端确认已认证时，`登录` / `Login` 禁用、`退出` / `Logout` 可点；服务端确认未认证或认证过期时，`登录` / `Login` 可点、`退出` / `Logout` 禁用。点击 `退出` / `Logout` 会清理当前插件共享的内存 Cookie 和 `%LocalAppData%\\OfficeAgent\\cookies.json` 中的持久化 Cookie，并把项目下拉框切换为 `请先登录` / `Sign in first`，但不会自动弹出登录提示或重新拉起登录流程。当前退出只清理插件本机会话，不调用远端 SSO logout endpoint。
...
- 如果项目接口成功返回，无论项目列表是否为空，账号组都会按服务端已认证状态刷新为 `登录` 禁用、`退出` 可点
- 当项目接口因未登录返回 `401/403` 时，会弹出本地化提示框：
  - 中文：`当前未登录，请先登录` + `点我登录`
  - 英文：`You're not signed in. Sign in first.` + `Sign in`
  用户可直接触发 Ribbon 登录；登录成功后会立即重载项目列表
- 当项目接口因未登录返回 `401/403` 时，插件会同时清理本地会话缓存，并把账号组刷新为 `登录` 可点、`退出` 禁用
- 由项目列表自动加载触发的未登录提示在同一个 Excel 进程内只弹出一次；如果用户直接关闭该提示，后续打开新的 workbook 或重新激活 workbook 时只保持 `请先登录` / `Sign in first` 状态，不再重复弹窗。用户登录成功后，该一次性提示状态会重置。
- 如果该未登录状态来自用户刚刚点击 `退出` / `Logout`，重新打开项目下拉框只保持 `请先登录` / `Sign in first` 状态，不再自动弹出登录提示；用户可手动点击账号组 `登录` / `Login` 重新登录
...
如果当前 sheet 是首次绑定，或者绑定项目变了，用户需要显式点击 `初始化当前表`。点击后不再直接执行初始化，而是先打开初始化对话框。

如果切换到了其他项目，插件会先清掉当前 sheet 原有的 `SheetFieldMappings`；在用户重新执行 `初始化当前表` 之前，下载和上传都不会静默自动初始化，而是直接报错要求先初始化。

`xISDP_Setting` 和 `xISDP_Log` 属于受管辅助工作表，禁止执行 `初始化当前表`，也禁止作为业务模板导入目标。用户在这些 sheet 上点击初始化时，只会看到禁止初始化提示，不会打开初始化对话框。
...
对话框打开时会为当前项目加载业务系统导出模板。当前 sheet 内容为空且业务模板可用时，默认选择 `从模板创建作业表` 并选中业务系统返回的第一个模板；当前 sheet 已有内容时，默认选择 `仅初始化配置`。如果当前连接器不支持业务导出模板、模板列表为空或模板列表加载失败，模板导入路径会禁用，用户仍可选择 `仅初始化配置` 完成原有初始化。非空 sheet 上仍可手动选择模板导入，但对话框会显示覆盖风险提示，主按钮会切换为 `覆盖并初始化`。
...
1. 按当前项目和模板 ID 调用业务系统导出接口下载 `.xlsx`。
2. 打开临时导出 workbook。
3. 查找名称必须精确等于 `Business Data` 的 worksheet。
4. 将 `Business Data` 的内容复制到当前业务 sheet，并保留当前业务 sheet 名称。
5. 写入当前项目的 `SheetBindings + SheetFieldMappings`。
6. 激活当前业务 sheet 并选中 `A1`。

业务导出模板只是初始化时的一次性输入，不会把业务模板 `templateId` 或 `templateName` 写入 `xISDP_Setting`。初始化成功后不会自动执行下载，不会自动打开任务窗格，也不会写入 `xISDP_Log`。

`Business Data` 复制目标是保留业务系统导出的作业表体验，包括单元格值、公式、格式、合并单元格、列宽、行高、隐藏行列和冻结窗格等 Excel sheet 级布局。第一版只导入 `Business Data` 这一张 sheet，不承诺保留导出 workbook 中的其他 sheet、VBA、外部连接、工作簿级名称、保护密码或跨 sheet 公式引用的完整可用性。

导入过程中，插件会把业务系统导出的临时 workbook 作为后台源文件打开，并尽量隐藏该源 workbook 窗口，避免用户看到下载的 Excel 文件闪到前台。用户可见焦点应保持在当前业务 workbook；完成后会重新激活当前业务 sheet 并选中 `A1`。冻结窗格读取依赖 Excel workbook window 状态，仍然属于 best-effort 复制。

模板导入会显示进度对话框。下载业务系统 `.xlsx` 阶段可以取消；用户取消后会中止 HTTP 下载、关闭进度对话框，不写 Setting、不改当前 sheet、不显示错误。进入 Excel COM 复制和 metadata 写入阶段后，第一版不支持安全中止。

导出或导入失败时不会自动降级为 `仅初始化配置`。如果下载失败、导出内容不是有效 `.xlsx`、找不到 `Business Data`、当前 workbook/sheet 保护导致无法写入，插件会提示错误并尽量保持当前 sheet 和 Setting 不变。如果业务内容已导入但最后 metadata 写入失败，插件会提示严重错误，说明表内容已导入但同步配置未完成，需要重新初始化当前表。
...
- 如果初始化过程中底层业务接口返回 `401/403`，不会继续显示普通错误，而是弹出本地化未登录提示框：
  - 中文：`当前未登录，请先登录` + `点我登录`
  - 英文：`You're not signed in. Sign in first.` + `Sign in`
- 同一次认证失败会同步刷新账号组按钮状态：`登录` 可点、`退出` 禁用
...
日志重写时会把 `xISDP_Log` 的写入区域固定为文本格式，并在读取已有日志时优先保留 Excel 当前显示文本；因此日期、前导零编号、百分比、长数字等业务值在后续追加日志时不会因为 Excel 的底层 `Value2` 转换而变成序列号或普通数字。`Changed At` 仍按插件固定时间格式归一化写入。
...
业务模板导出能力是可选扩展，不直接加入 `ISystemConnector` 主接口。当前扩展接口是：

- [src/OfficeAgent.Core/Services/IBusinessExportTemplateConnector.cs](../../src/OfficeAgent.Core/Services/IBusinessExportTemplateConnector.cs)

未实现该扩展的连接器仍可执行项目选择、`仅初始化配置`、下载和上传；初始化对话框只会禁用 `从模板创建作业表` 模式。
...
- `POST /templates`
  - 初始化当前表时加载业务系统导出模板列表
  - 按 `projectId` 返回当前项目可用模板
  - 返回项只要求包含稳定 `templateId` 和展示用 `templateName`
  - 当前 mock server 为每个项目返回 `standard / 标准作业表` 和 `review / 评审作业表`
- `POST /export`
  - 初始化当前表的模板导入路径使用
  - 按 `projectId + templateId` 返回 `.xlsx` 二进制文件
  - 响应 `Content-Type` 为 `application/vnd.openxmlformats-officedocument.spreadsheetml.sheet`
  - 下载文件名为 `business-export.xlsx`
  - 导出的 workbook 必须包含名称精确等于 `Business Data` 的 worksheet
  - 插件不要求导出接口额外返回字段映射 metadata；`SheetFieldMappings` 仍由连接器的 `BuildFieldMappingSeed()` 按当前项目初始化生成
- `POST /head`
  - 返回 `headList`
