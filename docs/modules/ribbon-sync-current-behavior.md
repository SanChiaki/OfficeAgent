...
Ribbon 分组、按钮和项目选择框状态文案都会跟随当前宿主 UI 语言切换。例如：
...
- 用户点击 Ribbon 项目选择入口后，会打开本地 WinForms 项目搜索选择弹窗
- 弹窗内项目列表通过连接器项目接口动态加载，不再使用本地硬编码项目列表
- 弹窗搜索框支持实时过滤，匹配规则包括大小写不敏感的包含匹配、多关键词匹配和非连续字符模糊匹配；例如 `tracker` 能匹配 `delivery-tracker-交付跟踪项目`，`dt` 也能匹配 `delivery-tracker`
- 项目条目文本显示为 `ProjectId-DisplayName`
- 弹窗搜索框输入只负责过滤候选项；只有用户确认选中某个项目条目时，才会触发项目切换和布局确认流程
- 如果当前 sheet 已有绑定，切换回来时项目选择入口会自动回填
- 即使项目列表尚未重新加载，项目选择入口也会先根据 `SheetBindings.ProjectId + ProjectName` 显示当前绑定项目
- 如果当前 sheet 没有绑定，项目选择入口显示 `先选择项目`
- 如果项目接口返回 `401 Unauthorized` 或 `403 Forbidden`，项目选择入口显示 `请先登录`
- 当项目接口因未登录返回 `401/403` 时，会弹出本地化提示框：
  - 中文：`当前未登录，请先登录` + `点我登录`
  - 英文：`You're not signed in. Sign in first.` + `Sign in`
  用户可直接触发 Ribbon 登录；登录成功后会立即重载项目列表
- 如果项目接口返回空数组，项目选择入口显示 `无可用项目`
- 如果项目接口出现其他异常，项目选择入口显示 `项目加载失败`
...
- 布局对话框点击取消会完全中止本次项目切换，并恢复项目选择入口到切换前项目状态
...
- Ribbon 不会直接根据底层 HTTP 状态码判断“未登录”；项目选择入口和同步动作都只认 `AuthenticationRequiredException`
