...
当前 Ribbon Sync 相关入口分为五组。Ribbon 组名、按钮名和项目下拉框状态文本会根据主机解析到的 UI 语言显示中文或英文；默认跟随 Excel UI 语言，设置中的语言覆盖值可强制使用 `zh` 或 `en`。

- 项目 / Project
  - 项目下拉框：`先选择项目` / `Select project`
...
`ISDP AI` 组中的任务窗格按钮固定显示为 `Open`。`文档` / `Documentation` 会用默认浏览器打开 `https://github.com/SanChiaki/OfficeAgent`；`关于` / `About` 会显示当前插件版本号、程序集版本、构建配置和构建时间。
...
- 如果当前 sheet 没有绑定，下拉框显示 `先选择项目` / `Select project`
- 如果项目接口返回 `401 Unauthorized` 或 `403 Forbidden`，下拉框显示 `请先登录` / `Sign in first`
- 当项目接口因未登录返回 `401/403` 时，会弹出 `当前未登录，请先登录` 提示框，用户可点击 `点我登录` 直接触发 Ribbon 登录；登录成功后会立即重载项目列表
- 如果项目接口返回空数组，下拉框显示 `无可用项目` / `No projects available`
- 如果项目接口出现其他异常，下拉框显示 `项目加载失败` / `Failed to load projects`
