...
账号组按钮状态跟随本地 SSO 会话 Cookie 联动：未登录时 `登录` / `Login` 可点、`退出` / `Logout` 禁用；已登录时 `登录` / `Login` 禁用、`退出` / `Logout` 可点。点击 `退出` / `Logout` 会清理当前插件共享的内存 Cookie 和 `%LocalAppData%\\OfficeAgent\\cookies.json` 中的持久化 Cookie，并把项目下拉框切换为 `请先登录` / `Sign in first`，但不会自动弹出登录提示或重新拉起登录流程。当前退出只清理插件本机会话，不调用远端 SSO logout endpoint。
