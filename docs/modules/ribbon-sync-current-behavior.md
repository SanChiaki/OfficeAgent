...
Release 安装包会在后台以非阻塞方式检查新版本。检查源是内部配置的独立 URL，不复用业务后端，也不显示在任务窗格设置页。发现比当前安装版本更高的新版本且用户未忽略该版本时，`关于` / `About` 图标会显示红点。点击 `关于` 会展示当前版本、最新版本、更新摘要和下载入口，并可选择 `忽略此版本` / `Ignore this version`；忽略后只隐藏红点，`关于` 弹框仍保留该版本的更新信息和下载入口，方便用户之后手动更新。没有新版本时显示 `当前已是最新版本。` / `You are on the latest version.`。Debug / 本地开发刷新环境不会请求更新源。更新检查失败只写本地日志，不弹窗，也不影响 Ribbon、任务窗格、登录、同步或模板操作。
...
`关于` / `About` 使用宿主生成的自定义图标，以便在存在新版本时叠加红点；其他 Ribbon 按钮仍使用 Office 内置 `imageMso` 图标。`初始化当前表` / `Initialize sheet`、`AI映射列` / `AI map columns`、`配置` / `Setting` 组内的三个命令按钮以及账号组内的 `登录` / `Login`、`登出` / `Logout` 使用 Office 常规小按钮布局；数据同步、帮助等其余命令按钮使用 Office 大按钮布局，图标显示在上方、文字显示在下方。项目下拉框仍然只显示当前项目或状态文本。

中文 UI 下，大按钮里的两个汉字 Ribbon 按钮标签会在文本末尾追加 carriage return no-wrap 提示，等价于 Ribbon XML 中的 `&#13;`，用于避免 Office 把 `下载`、`上传`、`文档`、`关于` 拆成逐字换行显示。账号组的 `登录`、`登出` 是 Office 常规小按钮，不追加该换行提示。该处理只发生在 Ribbon 按钮赋值层，不改变同步结果、确认弹窗等普通本地化文案。

账号组按钮状态跟随本地 SSO 会话 Cookie 联动：未登录时 `登录` / `Login` 可点、`登出` / `Logout` 禁用；已登录时 `登录` / `Login` 禁用、`登出` / `Logout` 可点。点击 `登出` / `Logout` 会清理当前插件共享的内存 Cookie 和 `%LocalAppData%\\OfficeAgent\\cookies.json` 中的持久化 Cookie，并把项目下拉框切换为 `请先登录` / `Sign in first`，但不会自动弹出登录提示或重新拉起登录流程。当前登出只清理插件本机会话，不调用远端 SSO logout endpoint。
...
- 如果该未登录状态来自用户刚刚点击 `登出` / `Logout`，重新打开项目下拉框只保持 `请先登录` / `Sign in first` 状态，不再自动弹出登录提示；用户可手动点击账号组 `登录` / `Login` 重新登录
