...
Release 安装包会在后台以非阻塞方式检查新版本。检查源是内部配置的独立 URL，不复用业务后端，也不显示在任务窗格设置页。发现比当前安装版本更高的新版本且用户未忽略该版本时，`关于` / `About` 图标会显示红点。点击 `关于` 会展示当前版本、最新版本、更新摘要和下载入口，并可选择 `忽略此版本` / `Ignore this version`；没有新版本时显示 `当前已是最新版本。` / `You are on the latest version.`。Debug / 本地开发刷新环境不会请求更新源。更新检查失败只写本地日志，不弹窗，也不影响 Ribbon、任务窗格、登录、同步或模板操作。
