...
推荐把自定义图片赋值集中放在 `AgentRibbon_Load`，避免图标配置散落在多个按钮初始化块里。保留现有本地化调用顺序：先应用本地化标签，再覆盖图片；项目下拉框使用 `ProjectDropDownPlaceholderText`，不要写成固定中文字符串。
...
private void ApplyCustomRibbonImages()
{
    ...
    SetRibbonImage(logoutButton, Properties.Resources.RibbonLogout);
...
- 保留 `ShowImage = true`。
- 不要在图片替换 helper 中统一覆盖 `ControlSize`。Designer 负责维护每个按钮的布局：`Initialize sheet`、`Setting` 组内的三个命令按钮以及账号组内的 `Login` / `Logout` 是 `RibbonControlSizeRegular`；数据同步和帮助等其余命令按钮仍是 `RibbonControlSizeLarge`。`xISDP AI` 任务窗格按钮保持大图标布局，按钮 `Label` 为空，并且设置 `ShowLabel = false`。
...
`AgentRibbon.Designer.cs` 当前包含按钮声明、分组、标签、内置图标和布局配置。Designer 中的标签应保持英文兜底值，运行时再由 `ApplyLocalizedLabels()` 根据 Excel UI 语言覆盖为中文或英文。

后续替换为自定义图片时有两种选择：

- 保留 Designer 中的按钮、分组、英文兜底标签、布局，只在 `AgentRibbon.cs` 的 `AgentRibbon_Load` 里覆盖图片。
...
不要通过 Designer 或图标初始化代码把 `Project`、`Data sync`、`Documentation` 等标签改回中文硬编码；否则英文 Excel 会再次出现中文按钮名。
...
- 按钮仍然 `ShowImage = true`
- 按钮保留各自配置的 `ControlSize`：`Initialize sheet`、`Apply Setting`、`Save Setting`、`Save as Setting` 以及账号组内的 `Login` / `Logout` 为 `RibbonControlSizeRegular`，数据同步和帮助等其余命令按钮为 `RibbonControlSizeLarge`
...
- `RibbonDesignerUsesEnglishSafeDefaultsForLocalizedControls`、`RibbonAppliesLocalizedLabelsFromHostStringsAtRuntime` 和 `HostLocalizedStringsTests` 仍然通过，确保图标替换没有破坏中英文 Ribbon 标签。
...
- 数据同步和帮助等命令按钮仍是上方图标、下方文字，账号组内 `Login` / `Logout` 仍是常规小按钮布局。
- 图片没有被拉伸、裁切、模糊或出现白底。
...
`Initialize sheet`、`Apply Setting`、`Save Setting`、`Save as Setting` 以及账号组内 `Login` / `Logout` 应保持 `RibbonControlSizeRegular`。数据同步和帮助等其余命令按钮应保持 `RibbonControlSizeLarge`，大按钮布局才会按 Office Ribbon 的常规方式显示为上方图标、下方文字。不要为了替换图片在 helper 中统一覆盖所有按钮的 `ControlSize`。`xISDP AI` 任务窗格按钮应继续保持空 `Label` 并设置 `ShowLabel = false`，避免重新显示 `Open` 文案。

### 英文 Excel 里又显示中文按钮名

这通常不是图标资源问题，而是图标替换时误改了 Ribbon 标签来源。优先检查：
