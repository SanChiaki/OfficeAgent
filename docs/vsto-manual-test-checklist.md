...
- Select or mock a project with a long `ProjectId-DisplayName`, then confirm the Ribbon project selector visible text is truncated after 20 display-width units with `…`, Chinese/non-ASCII characters count as width 2, and the full project text is available from the button ScreenTip and the project selector popup.
